...
module.exports = grammar(HTML, {
	...
	rules: {
		...
		then_start: ($) =>
			seq(
				'{',
				alias($._then_tag, $.block_tag),
				optional($.svelte_raw_text),
				'}',
			),
		...
		await_end: ($) =>
			seq('{', alias($._await_end_tag, $.block_end_tag), '}'),
		...
		key_start: ($) =>
			seq(
				'{',
				alias($._key_start_tag, $.block_start_tag),
				$.svelte_raw_text,
				'}',
			),
		...
		snippet_end: ($) =>
			seq('{', alias($._snippet_end_tag, $.block_end_tag), '}'),
		...
		html_tag: ($) =>
			seq('{', alias($._html_tag, $.expression_tag), $._tag_value, '}'),

		_const_tag: (_) => tag('@', 'const'),
		const_tag: ($) =>
			seq('{', alias($._const_tag, $.expression_tag), $._tag_value, '}'),

		_debug_tag: (_) => tag('@', 'debug'),
		debug_tag: ($) =>
			seq('{', alias($._debug_tag, $.expression_tag), $._tag_value, '}'),

		_render_tag: (_) => tag('@', 'render'),
		render_tag: ($) =>
			seq('{', alias($._render_tag, $.expression_tag), $._tag_value, '}'),
...
function tag(sym, text) {
	return seq(sym, field('tag', token.immediate(text)));
}
